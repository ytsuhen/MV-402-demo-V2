...
def build_pdf(pd_data: dict, score: float, status: str, M, S_rest, alpha,
    ...
    # Додаємо fontName="Roboto" до всіх стилів
    style_title = ParagraphStyle("vtitle", parent=styles["Title"], fontName="Roboto", fontSize=14, spaceAfter=4)
    style_sub   = ParagraphStyle("vsub",   parent=styles["Normal"], fontName="Roboto", fontSize=9, textColor=colors.grey)
    style_h2    = ParagraphStyle("vh2",    parent=styles["Heading2"], fontName="Roboto", fontSize=11, spaceBefore=10, spaceAfter=4)
    style_body  = ParagraphStyle("vbody",  parent=styles["Normal"], fontName="Roboto", fontSize=9, leading=13)
    style_stamp = ParagraphStyle("vstamp", parent=styles["Normal"], fontName="Roboto", fontSize=11, textColor=colors.red,
    ...
    if is_early_exit or tdv_fail:
        ...
        st_style = ParagraphStyle("vstatus", parent=style_stamp, fontName="Roboto", textColor=color_status)
    ...
    # ВАЖЛИВО: Ці два рядки формують і повертають сам файл!
    doc.build(story)
    return buf.getvalue()
